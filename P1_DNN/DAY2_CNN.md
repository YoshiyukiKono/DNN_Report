...
```
##### 解説
シグモイド関数の微分（導関数：微分係数を返す関数）は以下の式（シグモイド関数の微分は、シグモイド関数で表すことができる）
```
 (1−sigmoid(x))・sigmoid(x)
```
x=0 の場合、sigmoid(x)=0.5
```
(1−0.5)×0.5=0.25
```
...
バッチ正規化は主に標準化とアフィン変換から構成。入力データの分布を0平均1分散のガウス分布に変換し、アフィン変換によって適切な平均と分散を求める
...
正則化やドロップアウトによって過学習を抑えることができるが、バッチ正規化との違いを認識する必要がある。

下記によると、現論文では、これまでの正則化テクニックを不要にできる（正則化・Dropoutの必要性が下がる）という議論がされているとのこと。
...
- 勾配の緩やかな斜面に対して、最適値に近づける。
- 学習率が徐々に小さくなるので、鞍点問題（大域的最適解に近づきにくい）を引き起こすことがあった。
- 学習率が徐々に小さくなるので、学習率が一定の場合と比較して、収束が早く、収束しやすい。

##### RMSProp
- 局所最適解にならず、大局的最適解を得やすい
- ハイパーパラメータの調整が必要な場合が少ない
- 指数的移動平均を利用（鞍点問題への対応として）
...
##### リッジ回帰
- 正則化された線形回帰の一つで、線形回帰に「学習した重みの二乗の合計（L2正則化項）」を加えたもの」
- L2正則化項による正則化では重みは完全に０にはならない性質があるため、説明変数が非常に多い時にはモデルの解釈が複雑になるという欠点がある。

#### 確認テスト
Ｌ１正則化のグラフのポイント

ラッソでは、一つの方向に対して０となる値を導き出すことができる。グラフとしてはダイヤ状の図示（基準線に接する=０を導く）。

#### 例題チャレンジ
Ｌ２パラメータ正則化における最終的な勾配計算（を選択肢から選択）
```
grad += rate * param
```
- 微分形（`param**2`を含む選択肢のように、二乗がそのまま残るのはおかしい）
